...
impl AudioAnalysisService {
    pub async fn analyze(state: &AppState, project_id: i64, file_id: i64) -> AppResult<AudioAnalysis> {
        ...
        let source_modified_ns = metadata.modified().ok()
        ...
        AudioAnalysisRepository::save(
            &connection, &analysis, source_size, source_modified_ns, ANALYSIS_VERSION,
        )?;
        Ok(analysis)
    }
}
...
fn analyze_file(path: &Path, file_id: i64) -> AppResult<AudioAnalysis> {
    ...
    let track = format.default_track(TrackType::Audio)
        .ok_or_else(|| AppError::AudioAnalysis("el archivo no contiene una pista de audio".into()))?;
    ...
    let parameters = track.codec_params.as_ref()
        ...
        .clone();
    let sample_rate = parameters.sample_rate
        .ok_or_else(|| AppError::AudioAnalysis("sample rate no disponible".into()))?;
    let channels = parameters.channels.as_ref()
    ...
    loop {
        ...
        loudness.add_frames_f32(&samples)
            .map_err(|error| AppError::AudioAnalysis(error.to_string()))?;
        sample_count = sample_count.saturating_add((samples.len() / channels as usize) as u64);
        packet_peaks.push(samples.iter().fold(0.0_f32, |peak, sample| peak.max(sample.abs())));
    }

    if sample_count == 0 {
        return Err(AppError::AudioAnalysis("no se pudieron decodificar muestras".into()));
    }
    ...
    let true_peak_dbfs = if true_peak > 0.0 { Some(20.0 * true_peak.log10()) } else { None };
...
fn compact_waveform(peaks: &[f32]) -> Vec<f32> {
    if peaks.is_empty() { return Vec::new(); }
...
mod tests {
    ...
    fn waveform_is_bounded_and_preserves_peaks() {
        let source = (0..400).map(|index| index as f32 / 400.0).collect::<Vec<_>>();
