    ...
    pub fn update(
        ...
        if input.tasks.is_empty() || input.tasks.len() > 20 {
            return Err(AppError::InvalidFinishPlan(
                "se requieren entre 1 y 20 etapas".into(),
            ));
        }
        ...
        for task in &mut input.tasks {
            ...
            if labels
                .iter()
                .any(|label: &String| label.eq_ignore_ascii_case(&task.label))
            {
                return Err(AppError::InvalidFinishPlan(
                    "las etapas no pueden repetirse".into(),
                ));
            }
...
fn normalize_optional(
    ...
    if value.chars().count() > maximum {
        return Err(AppError::InvalidFinishPlan(format!(
            "{field} supera {maximum} caracteres"
        )));
    }
...
fn validate_date(value: Option<String>) -> AppResult<Option<String>> {
    let Some(value) = normalize_optional(value, 10, "fecha objetivo")? else {
        return Ok(None);
    };
    let parts = value
        .split('-')
...
mod tests {
    ...
    fn validates_dates_and_optional_copy() {
        assert_eq!(
            validate_date(Some("2026-08-20".into())).unwrap().as_deref(),
            Some("2026-08-20")
        );
        assert!(validate_date(Some("20/08/2026".into())).is_err());
        assert_eq!(
            normalize_optional(Some("  ".into()), 20, "campo").unwrap(),
            None
        );
    }
}
